...
当前正式版本为 `v1.3.0`。“星光成长册 + 学习完成报告”已经完成开发、自动测试和 Windows 真实浏览器验收；原始规划保留在第 20 章，最终落地状态和接手说明见第 21 章。

Windows 已完成真实启动、浏览器 OCR 和 `v1.3.0` 成长功能验收。用户已确认前期 `v1.1.1` 在真实 Mac 设备上能够正常启动和运行；`v1.3.0` 尚未在 Mac 上验证。

### 1.1 当前版本与 Git 基线

- 产品版本：`1.3.0`，以 `package.json` 和 `package-lock.json` 为版本事实源。
...
1. 阅读 `HANDOFF.md` 和 `README.md`，确认当前正式基线为 `v1.3.0`。
...
6. `v1.3.0` 必须先建立旧数据迁移、奖励幂等和备份恢复测试，再开发成长册及完成报告界面。
7. 所有 `v1.3.0` 自动测试和浏览器验收使用独立临时数据；不得读取、恢复或改写真实词库。
...
13. 汇报时分别写清：
   ...
   - Windows 真实浏览器是否验证。
   - Mac 是否验证。
14. 完成开发和验收后，再决定 `v1.3.0` 的版本提交、标签和 GitHub Release；不要在功能未完成时提前发布。
15. 完成后更新本文的功能状态、踩坑记录、Git 基线和验证边界。

## 19. 当前最终交付判断

> 本章以下内容是 `v1.2.0` 阶段的收尾记录。当前 `v1.3.0` 最终交付判断见第 21 章。
