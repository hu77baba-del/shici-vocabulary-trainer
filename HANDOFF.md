...
项目已经是一个可以在 Windows 上真实运行的本地英语背单词工具。1.1.0 新增了每日新词数量自定义和自主复习：首页显示全部未学习新词，每天由用户选择学习数量并固定当天词条；自主复习支持自动抽取、手动勾选、自由认读、自由拼写和正式拼写。照片 OCR 已从 Tesseract.js 完整替换为离线 PaddleOCR.js PP-OCRv5，三张回归样例分别形成 11 组、10 组和 7 组固定期望。词库、学习计划、拼写、错词循环、自主复习、发音和备份相关自动测试均通过。

Windows 已完成真实启动和浏览器 OCR 验收。Mac 启动脚本和网页代码已经准备好，但尚未在真实 Mac 设备上验收。

### 1.1 当前版本与 Git 基线

- 产品版本：`1.1.0`，以 `package.json` 和 `package-lock.json` 为版本事实源。
...
- 本交接文档已随第二次功能提交同步更新；后续接手仍应以工作区中的最新 `HANDOFF.md` 为准，并用 `git status` 判断是否又产生新的未提交变更。
...
当前 `1.1.0` 在 1.0.0 基础上完成了每日新词自定义和自主复习：用户每天选择新词数量，当天固定词条；已经学习过的词可自由认读、自由拼写或正式拼写。Windows 已使用独立临时词库完成真实浏览器验收，自动测试为 19 pass、0 fail。上述功能、首页视觉改进、测试和文档已纳入第二次 Git 提交；真实词库、备份和教材照片未进入版本库。
...
下一位接手者不需要重新论证“要不要换 PaddleOCR”，也不需要重新实现每日数量或自主复习。应把精力放在 Mac 实机验收、原始 JPEG 的用户浏览器复核、多图失败队列和长期使用体验上。
