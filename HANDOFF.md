...
当前正式版本为 `v1.3.1`。在 `v1.3.0`“星光成长册 + 学习完成报告”基础上，已经完成浏览器返回键修复、自动测试和 Windows Edge 真实浏览器验收；成长功能原始规划与落地记录见第 20、21 章，导航修复见第 22 章。

Windows 已完成真实启动、浏览器 OCR、`v1.3.0` 成长功能和 `v1.3.1` 返回键修复验收。用户已确认前期 `v1.1.1` 在真实 Mac 设备上能够正常启动和运行；`v1.3.1` 尚未在 Mac 上验证。

### 1.1 当前版本与 Git 基线

- 产品版本：`1.3.1`，以 `package.json` 和 `package-lock.json` 为版本事实源。
...
1. 阅读 `HANDOFF.md` 和 `README.md`，确认当前正式基线为 `v1.3.1`。
