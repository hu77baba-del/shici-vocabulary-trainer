...
- 第七次功能提交：`1930a13 feat: 发布 V1.4.0 听力认词模式`，包含独立听力认词、本地 CMU 发音相似度、听力错题重练和独立错误统计；已推送到远程 `origin/main`。
...
- 代码和文档版本已更新为 `1.4.0`；功能提交 `1930a13` 已推送到远程 `origin/main`，本章的实际提交与验证状态由后续文档收尾提交保存。
- 当前没有创建 Git 标签 `v1.4.0`，也没有创建 GitHub Release“拾词 v1.4.0”；本轮用户只要求本地提交、远程推送和 HANDOFF 更新，标签与 Release 需在用户明确要求发布后再执行。
