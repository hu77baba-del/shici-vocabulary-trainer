...
项目已经是一个可以在 Windows 上真实运行的本地英语背单词工具。1.1.0 新增了每日新词数量自定义和自主复习；1.1.1 继续增加词库多选批量删除，并修复透视页面整列错位、长释义错误分列、斜杠同义短语被过滤和错误旋转提示。“我的词库”支持跨搜索、筛选结果保留多选，删除时会同步清理相关学习记录和当天计划引用。照片 OCR 使用离线 PaddleOCR.js PP-OCRv5，回归期望覆盖 11 组短语、10 组多列表格、7 组星号词、26 组透视倾斜词表和两行斜杠同义短语。词库、学习计划、拼写、错词循环、自主复习、批量删除、发音和备份相关自动测试均通过。

Windows 已完成真实启动和浏览器 OCR 验收。Mac 启动脚本和网页代码已经准备好，但尚未在真实 Mac 设备上验收。

### 1.1 当前版本与 Git 基线

- 产品版本：`1.1.1`，以 `package.json` 和 `package-lock.json` 为版本事实源。
...
- 本交接文档已随 1.1.1 发布同步更新；后续接手仍应以工作区中的最新 `HANDOFF.md` 为准，并用 `git status` 判断是否又产生新的未提交变更。
...
当前应用内浏览器无法解码第二、第三张原始 JPEG，但能够处理由原图无损转换、像素内容一致的 PNG。改动后的自动转正、解析和界面流程已用这些 PNG 副本验证；原始 JPEG 仍需在用户实际使用的 Chrome/Edge 中复核。

这类情况下必须分别汇报“同像素内容已通过 OCR 流程”和“原始文件格式已在目标浏览器验证”，不能把两者合并说成原始 JPEG 已完整验收。
...
- 原始 JPEG `7ece03fa1b57cbb5a1a748fd19012f31.jpg` 完整输出 26 组，配对率 100%，从 `mascot / 吉祥物` 到 `disappoint / 使失望；使落空` 均逐行正确，0 个缺失、0 个建议检查。
...
- 首页功能区和横幅插画已在真实浏览器检查：1980×1248 视口无纵向或横向溢出，插画区域为 1268×317；390×844 手机视口无横向溢出，并使用偏左裁切保留学习人物。
...
13. 不要把同像素 PNG 副本通过写成原始 JPEG 已在目标浏览器通过。
...
2. 在用户实际使用的 Chrome/Edge 中用第二、第三张原始 JPEG 做最终复核。
...
当前 `1.1.1` 在 1.1.0 基础上增加了词库多选批量删除，并修复透视倾斜、长短释义混排造成的 OCR 整列错位；斜杠连接的两行短语可完整识别且不再误报旋转。用户每天选择新词数量、当天固定词条、自主复习和首页体验继续保持兼容。Windows 已使用独立临时词库完成真实浏览器验收，自动测试为 23 pass、0 fail。真实词库、备份和教材照片未进入版本库；Mac 实机仍未验收。
...
下一位接手者不需要重新论证“要不要换 PaddleOCR”，也不需要重新实现每日数量、自主复习或词库批量删除。应把精力放在 Mac 实机验收、原始 JPEG 的用户浏览器复核、多图失败队列和长期使用体验上。
